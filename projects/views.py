...
def project(request, pk):
    ...
    if request.method == 'POST':
        ...
        review.project = projects_obj
        review.owner = request.user.profile
        review.save()

        projects_obj.get_vote_count()

        messages.success(request, 'Ваш отзыв успешно отправлен')
        return redirect('project', pk=projects_obj.id)

    return render(request, 'projects/single-project.html', {'project': projects_obj, 'form': form})
